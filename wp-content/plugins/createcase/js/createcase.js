...
//    drawShape();
    jQuery('span.fileUpload').click(function() {
        jQuery('input[name=fileUpload]').trigger('click');
    });
});
...
var resizeableImage = function (image_target) {
    ...
    var $container,
            orig_src = new Image(),
            image_target = jQuery(image_target).get(0),
            event_state = {},
            constrain = false,
            min_width = 460, // Change as required
            min_height = 460,
    ...
    init = function () {
        ...
        $container = jQuery(image_target).parent('.resize-container');

        // Add events
        $container.on('mousedown touchstart', '.resize-handle', startResize);
        $container.on('mousedown touchstart', 'img', startMoving);
        jQuery('.js-crop').on('click', crop);
    };
    ...
    crop = function () {
        ...
        var crop_canvas,
                left = jQuery('.overlay').offset().left - $container.offset().left,
                top = jQuery('.overlay').offset().top - $container.offset().top,

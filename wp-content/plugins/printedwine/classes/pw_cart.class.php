...
if (!class_exists('PW_Cart')) {
    ...
    class PW_Cart {
        ...
        public function __construct() {
            ...
            //add additional [pw_data]([pw_product],[pw_price]) to cart item
            ...
            <script type="text/javascript">
                ...
                jQuery(document).ready(function () {
                    ...
                    jQuery('form .cart_item').each(function (i, item) {
                        ...
                        console.log($item.find('.product-thumbnail a'));
        ...
        public function change_cart_item_prices_text($price, $cart_item, $cart_item_key) {
            ...
            if (isset($cart_item['pw_data']) && $cart_item['pw_data']) {
                $pw_data = $cart_item['pw_data'];
...
//                }
        ...
        public function add_product_to_cart($cart_item_key, $product_id, $quantity, $variation_id, $variation, $cart_item_data) {
            ...
            if (isset($cart_item_data['pw_data'])) {

                //check if an old cart item exist
                if (!empty($cart_item_data['pw_data']['pw_remove_cart_item'])) {

                    global $woocommerce;
                    $woocommerce->cart->set_quantity($cart_item_data['pw_data']['pw_remove_cart_item'], 0);
                }
            }
        }
        ...
        public function add_cart_item($cart_item) {
            ...
            if (isset($cart_item['pw_data']) && $cart_item['pw_data']) {
                $pw_data = $cart_item['pw_data'];
                if (isset($pw_data['pw_product_price'])) {
                    $product = $cart_item['data'];
                    $product->adjust_price($pw_data['pw_product_price']);
...
//                    }
        ...
        public function add_cart_item_data($cart_item_meta, $product_id) {
            ...
            if (isset($_POST['pw_product'])) {

                $cart_item_meta['pw_data'] = array();
                $cart_item_meta['pw_data']['pw_product'] = $_POST['pw_product'];
                $cart_item_meta['pw_data']['pw_product_price'] = $_POST['pw_product_price'];
                $cart_item_meta['pw_data']['pw_product_thumbnail'] = $_POST['pw_product_thumbnail'];
                $cart_item_meta['pw_data']['pw_remove_cart_item'] = $_POST['pw_remove_cart_item'];
            }
        ...
        public function get_cart_item_from_session($cart_item, $values) {
            ...
            if (isset($values['pw_data'])) {
                $cart_item['pw_data'] = $values['pw_data'];
            }

            //check if cart item is fancy product
            if (isset($cart_item['pw_data'])) {
...
//		    if ( isset($cart_item['pw_data']) && fpd_get_option('fpd_cart_show_element_props') ) {
            if (isset($cart_item['pw_data']) && false) {

                //get fpd data
                $pw_data = $cart_item['pw_data'];

                if (isset($pw_data['pw_product']) && $pw_data['pw_product']) {
                    ...
                    $views = json_decode(stripslashes($pw_data['pw_product']), true);
                    ...
                    foreach ($views as $view) {
                        ...
                        foreach ($viewElements as $viewElement) {
                            ...
                            if (@$elementParams['isEditable']) {

                                $values = array();

                                //check if fill is set and if yes, look for a hex name
                                if (@$elementParams['fill'] && is_string($elementParams['fill'])) {
        ...
        public function reset_cart_item_link($link, $cart_item, $cart_item_key) {
            ...
            if (isset($cart_item['pw_data']) && $cart_item['pw_data']) {
                ...
                return sprintf('<a href="%s">%s<br /><i style="opacity: 1; font-size: 0.9em;">%s</i></a>', $url, $cart_item['data']->get_title(), "Click here to re-edit");
            }
        ...
        public function change_cart_item_thumbnail($thumbnail, $cart_item = null) {
            ...
            if (!is_null($cart_item) && isset($cart_item['pw_data'])) {

                $pw_data = $cart_item['pw_data'];

                //check if data contains the fancy product thumbnail
                if (isset($pw_data['pw_product_thumbnail']) && $pw_data['pw_product_thumbnail']) {
                    ...
                    foreach ($src as $s) {
                        $s->nodeValue = $pw_data['pw_product_thumbnail'];
                    }

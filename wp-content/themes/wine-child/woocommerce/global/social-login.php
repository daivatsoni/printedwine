...
<?php if ( $providers ) : ?>
    ...
    <?php /*<div class="wc-social-login form-row-wide">

        <p><?php echo wp_kses_post( $login_text ); ?></p> */ ?>

        <?php
            foreach ( $providers as $provider ) :
                printf( '<a href="%1$s" class="iconLogin fb icon-%2$s"></a> ', esc_url( $provider->get_auth_url( $return_url ) ), esc_attr( $provider->get_id() ) );
            endforeach;
        ?>
